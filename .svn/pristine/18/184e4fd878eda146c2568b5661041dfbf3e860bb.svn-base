 ...
 public class xuchuan
    {
        ...
        public void Begin(Stream sm,string filenames)
        {
            ...
            lli = (List<Locations.Locations>)bf.Deserialize(sm); //反序列化，得到续传信息
            dtbegin = DateTime.Now;                              //设置开始续传的时间，显示给客户
            if (lli.Count>0)
            {
                filesize = lli[lli.Count - 1].Filesize;          //得到文件的总大小

            }
            xiancheng = lli.Count;                              //判断续传需要多少线程
            string s = filenames;                               //续传文件名称
            fileNameAndPath = s.Substring(0, s.Length - 4);     //得到续传完成后，下载到本地文件的文件路径及名称
            filename = fileNameAndPath.Substring(fileNameAndPath.LastIndexOf(@"\") + 1, fileNameAndPath.Length - (fileNameAndPath.LastIndexOf(@"\") + 1));//得到文件名称
            new Set().GetConfig();
        ...
        public void GetData(object l)
        {
            ...
            try
            {
                ...
                Locations.Locations ll = (Locations.Locations)l;
        ...
        private void clear()
        {
            ...
            xiancheng = 0;                  //重置线程数量
            lli = new List<Locations.Locations>();//重置续传信息
